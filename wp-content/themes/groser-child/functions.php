...
add_action(
	...
	function () {
		...
		wp_enqueue_style( $parent_handle, get_template_directory_uri() . '/style.css', array(), wp_get_theme( 'groser' )->get( 'Version' ) );

		// Then child style, dependent on parent
		wp_enqueue_style( 'groser-child-style', get_stylesheet_uri(), array( $parent_handle ), wp_get_theme()->get( 'Version' ) );
	}
);
